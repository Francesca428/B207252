...
gt() table.
```{r}
may2020antibiotics_table <- may2020antibiotics_top10 %>%
  summarize(quantity_sum = sum(quantity_sum)) %>% 
  mutate(antibiotic_name = "Total") %>% 
  bind_rows(may2020antibiotics_top10, .)
  
  #gt() %>% 
  #cols_label(antibiotic_name = "Antibiotic",
             #quantity_sum = "Prescriptions") %>% 
   #cols_align(align = "center",
              #columns = antibiotic_name) %>% 
   #cols_align(align = "center",
              #columns = quantity_sum) %>% 
  #tab_style(
    #style = cell_text(weight = "bold"),
    #locations = cells_column_labels(columns = everything())
  #)
may2020antibiotics_table
```
...
```{r}
may2024antibiotics_table <- may2024antibiotics_top10 %>%
  summarize(quantity_sum = sum(quantity_sum)) %>% 
  mutate(antibiotic_name = "Total") %>% 
  bind_rows(may2024antibiotics_top10, .)
may2024antibiotics_table
```
...
```{r}
may2019antibiotics_table <- may2019antibiotics_top10 %>%
  summarize(quantity_sum = sum(quantity_sum)) %>% 
  mutate(antibiotic_name = "Total") %>% 
  bind_rows(may2019antibiotics_top10, .)
may2019antibiotics_table
```
