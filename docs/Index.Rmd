...
(Should I focus on one lockdown or also compare between lockdowns)?

First I will look load the appropriate libraries. Then, load, clean and filter the data.
```{r, include=FALSE}
library(tidyverse)
library(janitor)
library(gt)
...
```
Next I arranged them in descending order and looked at the top 10 most prescribed antibiotics.
```{r}
...
may2020antibiotics_top10 <- may2020antibiotics %>% 
  slice(1:10)
```
I prepared the pre- (2019) and post (2020)-lockdown data for merging, as I did before (I hid this to save space for the formative work. I will include it in the summative assessment.)
